    ...
    const updateChart = async () => {
        ...
        for (const treatment of ids) {
            ...
            reportData.datasets = [
                ...reportData.datasets,
                {
                    label: `${treatment[0].substr(-4, 4)}${treatment[1].substr(
                        -4,
                        4
                    )}`,
                    data: treatmentLog.map((value, index) => {
        ...
        const patientLegendPlugin = {
            ...
            afterUpdate(chart, args, options) {
                ...
                items.forEach((item) => {
                    // console.log(item);
                    if (!patients.includes(item.text.substr(4, 4))) {
                        const li = document.createElement("li");
                        li.style.alignItems = "center";
                        li.style.cursor = "pointer";
                        li.style.display = "flex";
                        ...
                        li.onclick = () => {
                            ...
                            if (type === "pie" || type === "doughnut") {
                                ...
                                chart.data.datasets.forEach((value, index) => {
                                    if (
                                        value.label.substr(4, 4) ===
                                        item.text.substr(4, 4)
                                    ) {
                                        // console.log(index);
                                        chart.setDatasetVisibility(
                                            index,
                        ...
                        // console.log(item);
                        const boxSpan = document.createElement("span");
                        boxSpan.style.background = `#${item.text.substr(
                            -6,
                            6
                        )}`;
                        ...
                        li.appendChild(textContainer);
                        ul.appendChild(li);
                        patients.push(item.text.substr(4, 4));
                    }
                });
            },
        };
