...
contract("Application", async (accounts) => {
  ...
  it(`should add ${accounts.length - 2} patients`, async () => {
    ...
    const bar1 = multiBar.create(accounts.length - 2, 0)
  ...
  it(`all patients accept doctor requests except patient 2 not accept doctor ${accounts.length - 2}`, async () => {
    ...
    for (let i = 0; i < accounts.length - 2; i++) {
      ...
      for (let j = 0; j < 2; j++) {
        ...
        // profile.acceptRequest(doctor, { from: patient })
        //   .then(res => {
        //     console.log(res);
        //     transactions.push(res)
        //   }).catch(err => console.log(`Patient ${i} has no request from doctor ${accounts.length - j - 1}`))
        // console.log(res);
        await Promise.all([
